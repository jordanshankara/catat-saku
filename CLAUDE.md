...
| D-28 | Tombol fitur fase berikutnya | Tombol yang fasenya belum dibuat tetap tampil sesuai aturan tampilnya; saat diketuk menampilkan pesan "dibuat di Fase N". Status setelah Fase 8: semua fitur bab 8 aktif, tidak ada lagi tombol "dibuat di Fase N". |
...
| D-30 | PIN | PBKDF2WithHmacSHA256, 120.000 iterasi, salt 16 byte. Hitungan salah PIN & batas jeda **disimpan permanen** (jeda tetap berlaku walau app ditutup paksa), di-reset saat PIN benar atau PIN baru dibuat. Numpad PIN tanpa tombol "000". |
...
| D-55 | Pengaturan | Lengkap sejak Fase 8: nama panggilan, pos & nominal (+ tambah/arsip, jatuh tempo), tema gelap, jam notifikasi, izin notifikasi & panduan baterai, ambang aman, target Dana Darurat, ganti PIN, batas kunci, Blokir screenshot, folder auto-backup, backup sekarang, Kirim backup, Pulihkan, versi app + Mode Uji. |
...
| D-64 | Bulan berlaku perubahan pos (R-95) | Bulan kalender berikutnya; jika bulan itu sudah punya alokasi (gaji cepat sudah di-split) atau sudah ditutup, maju ke bulan pertama yang belum. Nominal disimpan berversi per bulan, jadi bulan berjalan dan sebelumnya tidak berubah. |
| D-65 | Pos yang bisa diarsipkan / ditambah | Tambah: STOK atau TETAP (R-95). Arsip: pos STOK selain Transport dan pos TETAP. Makan, Buah, Transport (mode akhir pekan), dan Nabung tidak bisa diarsipkan karena aturan bab 6 bergantung padanya; nominalnya tetap bisa diubah. "Batalkan arsip" tersedia sebelum bulan arsip dimulai. |
| D-66 | Ikon pos baru | Memakai ikon & palet Lain-lain (bagian 12 "pos baru memilih dari palet ini"); key `pos_{id}`. |
| D-67 | Jatuh tempo pos TETAP | Perubahan tanggal jatuh tempo langsung berlaku (tidak mengubah angka uang), dibatasi 1–31; bulan yang lebih pendek memakai hari terakhirnya. |
| D-68 | Tema gelap | Sakelar di Pengaturan > Tampilan (default mati). Warna bab 12 tema gelap + token turunan dengan kontras ≥ 4.5:1. Tombol berteks putih memakai warna isi yang lebih gelap (`primaryFill #3563E9`, `successFill #17865A`, `dangerFill #B3243F`) di kedua tema karena `#5B8CFF`/`#3DD598`/`#D93A5A` + teks putih di bawah 4.5:1. Grafik memakai versi gelap dari palet D-60. |
| D-69 | Widget | Glance 1.2.0. Isi: sisa jatah pos HARIAN hari ini (berteks "Sisa/Pas/Lebih") + tombol "+ Catat". **Tanpa** Saku Sisa, Tabungan, atau saldo lain karena widget tampil di layar utama tanpa PIN. Ketuk baris → input pos itu, "+ Catat" → pemilih kategori; keduanya lewat kunci PIN (8.14). Diperbarui saat data berubah dan tiap jam (pergantian hari). |
| D-70 | Ganti PIN | Pengaturan > Keamanan: PIN lama → PIN baru → ketik ulang. Salah PIN lama memakai hitungan & jeda yang sama dengan layar kunci. |
| D-71 | Keamanan (audit Fase 8) | (1) Hash PIN & hitungan salah PIN dipindah ke DataStore terpisah `secure` yang dikecualikan dari Android Auto Backup / device transfer (PIN 4 digit mudah ditebak offline dari hash); data lama dimigrasi otomatis. (2) Rute dari notifikasi/widget wajib membawa token acak per instalasi + rute harus ada di daftar putih; intent dari app lain diabaikan. (3) `RescheduleReceiver` hanya memproses aksi sistem yang terdaftar. (4) Lama di-background diukur dengan jam monotonik, jadi memundurkan jam HP tidak melewati kunci. (5) File backup divalidasi isinya (enum, tanggal, referensi pos, nominal 0..999.999.999.999, ukuran ≤ 20 MB, ledger harus bisa dihitung) sebelum Pulihkan. (6) File share di cache dihapus setelah 1 jam. Sisa risiko yang sesuai spesifikasi: database dan file backup tidak terenkripsi. |
| D-72 | Izin dari Glance | Glance 1.2.0 membawa WorkManager yang menambah `WAKE_LOCK`, `ACCESS_NETWORK_STATE`, `FOREGROUND_SERVICE`. Ketiganya dihapus dengan `tools:node="remove"` (batas izin bab 3): widget tidak memakai syarat jaringan, foreground/expedited work, dan jalur wakelock WorkManager hanya untuk API < 23. Semua komponen exported bawaan library dilindungi izin sistem (BIND_REMOTEVIEWS, BIND_JOB_SERVICE, DUMP). |
| D-73 | Bulan di tab Riwayat | Tab Riwayat mengingat bulan & filter terakhir yang dilihat, tetapi pindah ke bulan berjalan begitu bulan baru dimulai (ditemukan saat simulasi hidup nyata Sep–Feb). |
